...
<div id="grid">
  ...
  <div class="panel">
    ...
    <div class="panel-body">
      <div id="map"></div>
      <div id="buoy-list"><div id="buoy-list-title">&#9650; BUOYS</div><div id="buoy-rows"></div></div>
      <div id="gps-overlay">
...
function drawLidar(data){const w=lidarCanvas.offsetWidth||400;const h=lidarCanvas.offsetHeight||400;lidarCanvas.width=w;lidarCanvas.height=h;const cx=w/2,cy=h/2;const maxR=data.range_max||10;const scale=Math.min(w,h)*0.45/maxR;lidarCtx.fillStyle='#020802';lidarCtx.fillRect(0,0,w,h);lidarCtx.strokeStyle='#0a2a0a';lidarCtx.lineWidth=1;lidarCtx.font='10px Courier New';lidarCtx.fillStyle='#1a5a1a';for(let i=1;i<=4;i++){const r=maxR*i/4;lidarCtx.beginPath();lidarCtx.arc(cx,cy,r*scale,0,Math.PI*2);lidarCtx.stroke();lidarCtx.fillText((r%1===0?r:r.toFixed(1))+'m',cx+3,cy-r*scale+11)}lidarCtx.strokeStyle='#0d3a0d';lidarCtx.beginPath();lidarCtx.moveTo(cx,4);lidarCtx.lineTo(cx,h-4);lidarCtx.moveTo(4,cy);lidarCtx.lineTo(w-4,cy);lidarCtx.stroke();lidarCtx.strokeStyle='#005500';lidarCtx.lineWidth=2;lidarCtx.beginPath();lidarCtx.moveTo(cx,cy-18);lidarCtx.lineTo(cx-4,cy-12);lidarCtx.moveTo(cx,cy-18);lidarCtx.lineTo(cx+4,cy-12);lidarCtx.stroke();lidarCtx.fillStyle='#00aa00';lidarCtx.beginPath();lidarCtx.arc(cx,cy,4,0,Math.PI*2);lidarCtx.fill();lidarCtx.fillStyle='#00ff00';for(const pt of data.points){if(!pt)continue;const sx=cx-pt[1]*scale;const sy=cy-pt[0]*scale;if(sx<0||sx>w||sy<0||sy>h)continue;lidarCtx.fillRect(sx-1,sy-1,2,2)}}
...
const BUOY_CSS={red:'#ff4444',green:'#00dd44',yellow:'#ffcc00',black:'#888888',white:'#eeeeee',blue:'#4488ff',unknown:'#666666'};
let latestBuoys=[];const buoyMapMarkers=[];
async function updateBuoys(){try{const res=await fetch('/api/buoys');if(res.ok){const data=await res.json();latestBuoys=Array.isArray(data)?data:[];_refreshBuoyMap();_refreshBuoyList()}}catch{}setTimeout(updateBuoys,250)}
function _refreshBuoyMap(){buoyMapMarkers.forEach(m=>map.removeLayer(m));buoyMapMarkers.length=0;for(const b of latestBuoys){if(b.lat===null||b.lon===null)continue;const c=BUOY_CSS[b.color]||BUOY_CSS.unknown;const m=L.circleMarker([b.lat,b.lon],{color:c,fillColor:c,fillOpacity:.85,radius:9,weight:2}).bindTooltip('<b style="color:'+c+'">'+(b.color||'?').toUpperCase()+'</b><br>'+b.range.toFixed(1)+'m &nbsp; '+b.bearing_deg.toFixed(0)+'°',{permanent:false,direction:'top'}).addTo(map);buoyMapMarkers.push(m)}}
function _refreshBuoyList(){const rows=document.getElementById('buoy-rows');if(!rows)return;rows.innerHTML='';if(!latestBuoys.length){rows.innerHTML='<div style="color:#333">none detected</div>';return}for(const b of latestBuoys){const c=BUOY_CSS[b.color]||BUOY_CSS.unknown;const el=document.createElement('div');el.className='brow';el.innerHTML='<div class="bdot" style="background:'+c+';border:1px solid '+c+'88"></div><div class="binfo">'+(b.color||'?')+' '+b.range.toFixed(1)+'m '+b.bearing_deg.toFixed(0)+'°</div>';rows.appendChild(el)}}
const _drawLidarBase=drawLidar;function drawLidar(data){_drawLidarBase(data);if(!latestBuoys.length)return;const w=lidarCanvas.offsetWidth||400;const h=lidarCanvas.offsetHeight||400;const cx=w/2,cy=h/2;const maxR=data.range_max||10;const scale=Math.min(w,h)*0.45/maxR;lidarCtx.font='9px Courier New';for(const b of latestBuoys){const rad=b.bearing_deg*Math.PI/180;const bx=b.range*Math.cos(rad);const by=b.range*Math.sin(rad);const sx=cx-by*scale;const sy=cy-bx*scale;if(sx<0||sx>w||sy<0||sy>h)continue;const c=BUOY_CSS[b.color]||BUOY_CSS.unknown;lidarCtx.fillStyle=c;lidarCtx.strokeStyle='rgba(0,0,0,0.7)';lidarCtx.lineWidth=1;lidarCtx.beginPath();lidarCtx.arc(sx,sy,6,0,Math.PI*2);lidarCtx.fill();lidarCtx.stroke();lidarCtx.fillStyle='#000';lidarCtx.fillText((b.color||'?')[0].toUpperCase()+' '+b.range.toFixed(1)+'m',sx+8,sy+4)}}
updateGPS();updateCamera();updateSeg();updateLidar();updateOdom();updateDetections();updateBuoys();
</script>
</body>
</html>
WEBUI_HTML_EOF
