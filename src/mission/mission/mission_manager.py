...
class MissionManager(Node):
    ...
    def _start_cb(self, req, res):
        if len(req.waypoints) == 0:
            res.success = False
            res.message = "latitudes and longitudes must be non-empty and equal length"
            return res
        ...
        self._waypoints = list(zip(req.latitudes, req.longitudes))
        self._idx = 0
        self._navigating = False
        self._active = True
        self.get_logger().info(f"Mission started: {len(self._waypoints)} waypoints")
    ...
    def _send_next(self):
        lat, lon = self._waypoints[self._idx]
        self.get_logger().info(f"WP {self._idx + 1}/{len(self._waypoints)}: ({lat}, {lon})")
        self._navigating = True
        req = FromLL.Request()
