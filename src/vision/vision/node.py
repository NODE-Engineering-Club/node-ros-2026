...
ROS parameters:
  model_path   (string, default "/models/yolo26n-seg-navier.onnx")
  confidence   (double, default 0.5)
  nms_iou      (double, default 0.45)
...
class VisionNode(Node):
    def __init__(self):
        ...
        self.declare_parameter("model_path", "/workspace/models/yolo26n-seg-navier.onnx")
        ...
        self._nms_iou = self.get_parameter("nms_iou").get_parameter_value().double_value
    ...
    def _postprocess(self, raw, scale):
        ...
        indices = cv2.dnn.NMSBoxes(
            boxes.tolist(), confidences.tolist(), self._confidence, self._nms_iou
        )
        if len(indices) == 0:
            return []
