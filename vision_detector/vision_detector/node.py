...
class VisionNode(Node):
    def __init__(self):
        ...
        # Infer nc from output shape: (1, 4 + nc + 32, num_anchors)
        self._nc = self.session.get_outputs()[0].shape[1] - 36
    ...
    def _postprocess(self, raw, scale):
        ...
        """
        preds = raw[0].T  # (num_anchors, 4 + nc + 32)
        box_cxcywh = preds[:, :4]
        ...
        indices = cv2.dnn.NMSBoxes(boxes.tolist(), confidences.tolist(), CONFIDENCE, NMS_IOU)
        if len(indices) == 0:
            return []

        return [(boxes[i], float(confidences[i]), int(class_ids[i])) for i in indices.flatten()]
